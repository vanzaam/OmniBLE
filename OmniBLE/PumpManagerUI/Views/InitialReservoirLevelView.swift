            ...
            Section(
                ...
                if let currentLevel = viewModel.currentReservoirLevel {
                    HStack {
                        ...
                        Spacer()
                        Text("\(Double(truncating: currentLevel as NSNumber), specifier: "%.2f") U")
...
class InitialReservoirLevelViewModel: ObservableObject {
    ...
    private let reservoirKey = "OmniBLE_initialReservoirLevel"
    ...
    func loadSettings() {
        // Load from pumpManager state
        initialReservoirLevel = Decimal(pumpManager.initialReservoirValue)
    ...
    func save() {
        ...
        if let value = initialReservoirLevel {
            let valueDouble = Double(truncating: value as NSNumber)
            pumpManager.initialReservoirValue = valueDouble
            print("🎯 InitialReservoirLevelView: Saved to pumpManager: \(value)")
        }
    ...
    private func calculateCurrentReservoirLevel() {
        // Get delivered insulin from pump
        if let delivered = pumpManager.state.podState?.lastInsulinMeasurements?.delivered,
           let initial = initialReservoirLevel {
            let initialDouble = Double(truncating: initial as NSNumber)
            currentReservoirLevel = Decimal(max(0, initialDouble - delivered))
        } else {
